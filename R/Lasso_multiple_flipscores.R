...
#' @title Multiple Lasso Selection and Flipscores
#' @description Applica `Lasso_flipscores` su più variabili in parallelo o sequenziale.
#' @param response_var_name Nome variabile risposta.
#' @param data Dataset completo.
#' @param vars_to_test Vettore di variabili da testare (se NULL ne testa tutte eccetto Y).
#' @param n_comp Componenti per Lasso.
...
#' @param parallel Booleano per calcolo parallelo.
#' @param n_cores Numero core (se parallel=TRUE).
#' @param max_cores Limite massimo core.
#' @param ... Altri parametri.
#' @return Lista di risultati per ogni variabile testata, inclusi p-value corretti (MaxT).
#' @export
Lasso_multiple_flipscores <- function(response_var_name, 
                                      data,
                                      vars_to_test = NULL,
  ...
  if(!isTRUE(parallel)) {
    ...
    if(length(vars_to_test) == 1) {
      ...
      results <- Lasso_flipscores(response_var_name, reference_var = vars_to_test, 
      ...
      results <- lapply(vars_to_test_pos, function(i) Lasso_flipscores(response_var_name, reference_var = names(data)[i],
    ...
    results <- future.apply::future_lapply(vars_to_test_pos, function(i) {
      Lasso_flipscores(response_var_name, reference_var = names(data)[i],
                       x = data[,-c(response_var_pos,i)],
                       y = data[,response_var_pos], data = data,
                       n_comp = n_comp, family = family,
                       score_type = score_type, seed = seed,
